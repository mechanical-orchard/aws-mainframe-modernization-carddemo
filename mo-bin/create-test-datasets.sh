...
allocate_job_id=$(zowe zos-jobs submit local-file $script_dir/ALLOCATE_TEST_DATASETS.jcl --rfj --host $HOST --port $PORT --user $IBMUSER --reject-unauthorized false --password $PASSWORD | jq -r .data.jobid)
