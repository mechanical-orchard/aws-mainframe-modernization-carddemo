...
allocate_job_id=$(zowe zos-jobs submit local-file $repo_dir/samples/jcl/BMSCMP.jcl --rfj --host $HOST --port $PORT --user $IBMUSER --reject-unauthorized false --password $PASSWORD | jq -r .data.jobid)
